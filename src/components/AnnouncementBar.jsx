...
import { ffLive, ffCurrent, ffCountdownTo, ffFormat, FF_HEADLINE, FF_FLAT, FF_WINDOWS, FF_GIFT, FF_SITEWIDE_WEEKS, ffTitle } from '@/lib/freakyFridays';
...
export default function AnnouncementBar() {
  ...
  useEffect(() => {
    ...
    if (ffLive(now)) {
      ...
      setFfActive(true);
      promos.unshift(
        `${w.name ? `\u26BE  ${w.name}` : `\uD83C\uDF83  FREAKY FRIDAYS \u2014 ${ffTitle(w)}`} is LIVE \u00B7 ${FF_GIFT[w.week] ? `${FF_GIFT[w.week]} \u00B7 ` : ''}${FF_FLAT ? '' : 'up to '}${FF_HEADLINE}% OFF ${FF_SITEWIDE_WEEKS.includes(w.week) ? 'SITEWIDE' : 'SELECT PRODUCTS'}, no code needed \u00B7 stack your creator code for even more${left ? ` \u00B7 ends in ${left}` : ''}`,
      );
    }
