  ...
  return (
    <main
      style={{
        ...
        padding: '40px 20px',
        background:
          'radial-gradient(ellipse at 60% 40%, rgba(34,197,94,0.18) 0%, rgba(168,85,247,0.10) 40%, transparent 70%), var(--bg-dark, #020617)',
      }}
    >
      <div
        style={{
      ...
      >
        ...
        <div
          style={{
            ...
            border: '1px solid var(--glass-border, rgba(168,85,247,0.12))',
            color: '#A855F7', fontSize: 12, letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 600,
          }}
        >
          <Lock size={13} /> Affiliates Only
        </div>

        <h1 style={{ fontSize: 26, fontWeight: 800, margin: '0 0 8px', color: 'var(--text-light, #F3EFFC)', letterSpacing: '-0.01em' }}>
          Affiliate Access
        </h1>
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <input
            ...
            style={{
              ...
              border: `1px solid ${err ? '#4ADE80' : 'var(--glass-border, rgba(168,85,247,0.12))'}`,
              borderRadius: 12, color: 'var(--text-light, #F3EFFC)', outline: 'none',
            }}
          />
          {err ? (
            <div style={{ color: '#4ADE80', fontSize: 14, textAlign: 'left', margin: '-4px 2px 0' }}>{err}</div>
          ) : null}
          <button
            type="submit"
            disabled={loading || !pw}
            style={{
              ...
              boxShadow: '0 0 20px rgba(168,85,247,0.35)', transition: 'opacity .15s',
            }}
          >
        ...
        <p style={{ margin: '22px 0 0', color: 'var(--text-muted, #4b5a72)', fontSize: 13 }}>
          ...
          <a href="mailto:support@ironwithin.io" style={{ color: '#A855F7', textDecoration: 'none' }}>
            Ask us to join
          </a>
        </p>
      </div>
