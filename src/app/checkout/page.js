...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = () => {
    ...
    const input = buildCheckoutInput({
      ...
      shipping,
      transactionId: '', // Stripe integration: put paymentIntent.id here
    });
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
              ...
              <Link href="/shop" style={{
                display: 'inline-flex', alignItems: 'center', gap: '7px',
                padding: '12px 24px', border: '1px solid var(--glass-border)',
                borderRadius: '10px', color: 'var(--text-light)', fontWeight: 500,
                textDecoration: 'none', fontFamily: 'var(--font-body)',
              }}>
                Continue Shopping
              </Link>
      ...
      <div className="container" style={{ paddingTop: '40px' }}>
            ...
            <div key={step} style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 14px', borderRadius: '10px', background: i === currentStep ? 'rgba(0,207,255,0.1)' : 'transparent' }}>
                <div style={{
                  ...
                  background: i < currentStep ? 'rgba(52,211,153,0.2)' : i === currentStep ? 'var(--gradient-primary)' : 'var(--card-dark)',
                  border: `2px solid ${i < currentStep ? '#34d399' : i === currentStep ? 'transparent' : 'var(--glass-border)'}`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '0.75rem', fontWeight: 700, color: i < currentStep ? '#34d399' : '#fff',
                }}>
                  {i < currentStep ? <CheckCircle2 size={14} /> : i + 1}
                </div>
                <span style={{ fontSize: '0.85rem', fontWeight: i === currentStep ? 600 : 400, color: i === currentStep ? 'var(--primary-blue)' : i < currentStep ? '#34d399' : 'var(--text-muted)' }}>
                  {step}
                </span>
              </div>
              {i < steps.length - 1 && (
                <div style={{ width: 32, height: 1, background: i < currentStep ? '#34d399' : 'var(--glass-border)' }} />
              )}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,3fr) minmax(0,2fr)', gap: '32px', alignItems: 'start' }}>
              ...
              <FormCard icon={<CheckCircle2 size={17} color="#34d399" />} title="Review Your Order">
                ...
                <button
                  ...
                  style={{
                    ...
                    boxShadow: 'var(--glow-blue)', opacity: placingOrder ? 0.8 : 1,
                  }}
                >
                  {placingOrder ? (
                    <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> Placing Order…</>
                  ) : (
                    <><Lock size={15} /> Place Order — {cartTotal}</>
                  )}
                </button>
              </FormCard>
            )}
          ...
          <div style={{
            ...
            <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '1rem', fontWeight: 800, marginBottom: '18px' }}>
              Order Summary
            </h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
              {cartItems.map(item => (
                <div key={item.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1, minWidth: 0 }}>
                    <div style={{
                      ...
                      {item.product?.node?.image?.sourceUrl ? (
                        <img src={item.product.node.image.sourceUrl} alt=""
                  ...
                  <span style={{ fontWeight: 600, fontSize: '0.85rem', flexShrink: 0 }}>{item.total}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '14px' }}>
              ...
              {cart?.appliedCoupons?.map(c => (
                <SummaryRow key={c.code} label={`Coupon: ${c.code}`} value={`-${c.discountAmount}`} valueColor="#34d399" />
              ))}
            </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        style={{ width: '100%', padding: '11px 14px', background: 'var(--bg-dark)', border: '1px solid var(--glass-border)', borderRadius: '10px', color: 'var(--text-light)', fontFamily: 'var(--font-body)', fontSize: '0.9rem', outline: 'none' }}
