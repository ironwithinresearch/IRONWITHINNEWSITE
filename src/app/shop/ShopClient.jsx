...
export default function ShopClient({ initialProducts = [] }) {
  ...
  // Seed from the server-prefetched list (ISR) so products paint instantly on the
  // default view; once the live useQuery resolves it takes over (and drives filters/search).
  const rawNodes = data?.products?.nodes ?? ((!activeCategory && !search) ? initialProducts : []);
  const products = [...rawNodes].filter(p => p.slug !== 'gift-card' && !p.slug.includes('-month-plan')).sort((a, b) =>
