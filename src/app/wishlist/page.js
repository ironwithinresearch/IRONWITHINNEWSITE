...
export default function WishlistPage() {
  ...
  // Strip HTML entities from price (e.g. "₨&nbsp;18" → "₨18" or strip currency)
        ...
        {wishlistItems.length === 0 ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
              ...
              <button onClick={clearWishlist} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '9px 16px', background: 'transparent', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 'var(--radius-md)', color: '#f87171', fontWeight: 500, fontSize: '0.82rem', cursor: 'pointer', fontFamily: 'var(--font-body)', transition: 'all 0.15s ease' }}
            ...
            {outOfStockCount > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', marginBottom: '20px', background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.2)', borderRadius: 'var(--radius-md)' }}>
                ...
                <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                  <strong style={{ color: '#fbbf24' }}>{outOfStockCount} item{outOfStockCount > 1 ? 's' : ''}</strong> in your wishlist {outOfStockCount > 1 ? 'are' : 'is'} currently out of stock.
                </p>
              </div>
            )}
              ...
              {wishlistItems.map(item => (
                <div key={item.id} style={{ background: 'var(--card-dark)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', display: 'flex', flexDirection: 'column', transition: 'all 0.2s ease', opacity: item.stockStatus !== 'IN_STOCK' ? 0.8 : 1 }}
                  ...
                  <div style={{ position: 'relative', background: 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '160px', borderBottom: '1px solid var(--glass-border)', overflow: 'hidden' }}>
                    ...
                    <button onClick={() => removeFromWishlist(item.id)} style={{ position: 'absolute', top: '10px', right: '10px', width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: '8px', color: 'var(--text-muted)', cursor: 'pointer', backdropFilter: 'blur(8px)' }}
                  ...
                  <div style={{ padding: '16px', flex: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button
                        ...
                        disabled={item.stockStatus !== 'IN_STOCK' || cartAdded[item.id]}
